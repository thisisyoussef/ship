...
- **ADR-ID**: ADR-0035
- **Date**: 2026-03-19
- **Context**: FleetGraph V2 had already become the real execution engine for several surfaces, but canonical routes, the worker, and the web client still depended on V1-shaped wrappers, confirm-only interrupts, and adapter layers. That left the rollout half-native and kept readiness/docs out of sync with real behavior.
- **Decision**: Promote one native FleetGraph V2 contract across the canonical API, worker, and web surfaces. The shared contract centers on `responsePayload`, `reasonedFindings`, `actionDrafts`, `pendingApproval`, `dialogSpec`, and structured resume input. Follow-up turns stay on the same V2 thread/checkpoint path, shared registry-backed action drafts drive both confirm and typed-dialog HITL flows, and proactive findings/action outcomes persist through the existing FleetGraph finding and action tables.
- **Alternatives Considered**: Keep translating V2 back into V1-shaped route/web payloads; preserve a V1 worker adapter indefinitely; add a second persistence surface for native V2 instead of reusing the current finding/action tables.
- **Consequences**: FleetGraph now has one end-to-end native contract for canonical surfaces, which reduces adapter drift and unlocks richer HITL flows, but OpenAPI, tests, and rollout docs must stay aligned with that contract and legacy helper modules should not silently become primary surfaces again.
