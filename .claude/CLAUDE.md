...
The `.ai/skills/software-factory/` directory contains a 10-skill pipeline for generating features from specs. Read the relevant SKILL.md before executing any factory stage.

### Available Commands

| Command | Skill | What It Does |
|---------|-------|-------------|
| "index this repo" | `repo-constitution` | Extract conventions to `.factory/CONSTITUTION.md` |
| "build [entity] from spec" | `factory-orchestrator` | Full pipeline: spec → PR |
| "parse this spec" | `feature-model` | Enrich spec with codebase context |
| "plan this feature" | `execution-planner` | Create ordered task DAG |
| "generate migration" | `migration-gen` | Safe DB migration + schema contract |
| "generate API" | `api-gen` | Backend code + API contract |
| "set up config" | `config-gen` | Feature flags, permissions, env vars |
| "generate frontend" | `frontend-gen` | UI components from API contract |
| "generate tests" | `test-suite-gen` | Multi-layer test suite |
| "assemble PR" | `pr-assembler` | Verification + PR + QA checklist |

### Typed Contracts

Skills communicate via JSON contracts in `.factory/current/`:
- `feature-model.json` → consumed by all generation stages
- `schema-contract.json` → migration-gen → api-gen
- `api-contract.json` → api-gen → frontend-gen, test-suite-gen
...
```json
{
  "entity": "EntityName",
  "fields": [
    {"name": "field_name", "kind": "string|number|boolean|enum|date"},
    {"name": "status", "kind": "enum", "values": ["active", "inactive"]}
  ],
  "permissions": "entity_name:manage",
  "audit": true,
  "integrations": ["vault"],
...
See `.ai/skills/software-factory/spec-template.json` for the full template with all options.
