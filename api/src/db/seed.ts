...
// ────────────────────────────────────────────────────────────────────────────
// Comprehensive FleetGraph demo data seeder
// Creates data configurations that exercise every feature:
//   1. Active sprint with issues (rich analysis data)
//   2. Sprint with 4 unassigned issues (triggers unassigned_issues finding)
//   3. Sprint with no owner (triggers sprint_no_owner finding)
//   4. Sprint with submitted plan awaiting approval (approve action demo)
//   5. Comments and standups for evidence_lookup tool
// ────────────────────────────────────────────────────────────────────────────
interface ComprehensiveDemoInput {
  currentSprintNumber: number
  demoOwner: { id: string; email: string; name: string }
...
async function seedFleetGraphComprehensiveDemo(
  pool: pg.Pool,
  input: ComprehensiveDemoInput
) {
  const { currentSprintNumber, demoOwner, emailToUserId, programId, projectId, workspaceId } = input
  ...
  async function ensureDemoSprint(title: string, properties: Record<string, unknown>): Promise<string> {
    ...
    await pool.query(
      `INSERT INTO document_associations (document_id, related_id, relationship_type, metadata)
       VALUES ($1, $2, 'program', '{"created_via":"fleetgraph_demo"}'::jsonb)
       ON CONFLICT (document_id, related_id, relationship_type) DO NOTHING`,
      [id, programId]
    )
    return id
  ...
  // Helper: idempotent issue creation with sprint + project + program associations
  async function ensureDemoIssue(
    ...
    const existing = await pool.query(
      `SELECT d.id FROM documents d
       JOIN document_associations da ON da.document_id = d.id
         AND da.related_id = $3 AND da.relationship_type = 'sprint'
       WHERE d.workspace_id = $1 AND d.document_type = 'issue' AND d.title = $2`,
      [workspaceId, title, sprintId]
    ) as { rows: Array<{ id: string }> }
    ...
    const inserted = await pool.query(
      `INSERT INTO documents (workspace_id, document_type, title, properties)
       VALUES ($1, 'issue', $2, $3::jsonb) RETURNING id`,
      [workspaceId, title, JSON.stringify(properties)]
    ) as { rows: Array<{ id: string }> }

    const id = inserted.rows[0]!.id
    // Sprint, project, and program associations
    for (const [relId, relType] of [[sprintId, 'sprint'], [projectId, 'project'], [programId, 'program']] as const) {
      await pool.query(
        `INSERT INTO document_associations (document_id, related_id, relationship_type, metadata)
         VALUES ($1, $2, $3, '{"created_via":"fleetgraph_demo"}'::jsonb)
         ON CONFLICT (document_id, related_id, relationship_type) DO NOTHING`,
        [id, relId, relType]
      )
    }
  ...
  // Helper: idempotent standup creation
  async function ensureDemoStandup(sprintId: string, userId: string, content: Record<string, unknown>) {
    const existing = await pool.query(
      `SELECT id FROM documents WHERE workspace_id = $1 AND document_type = 'standup'
       AND properties->>'sprint_id' = $2 AND properties->>'user_id' = $3`,
      [workspaceId, sprintId, userId]
    ) as { rows: Array<{ id: string }> }
    if (existing.rows[0]) return
    await pool.query(
      `INSERT INTO documents (workspace_id, document_type, title, properties, content)
       VALUES ($1, 'standup', $2, $3::jsonb, $4::jsonb)`,
      [
        workspaceId,
        'Daily Update',
        JSON.stringify({ sprint_id: sprintId, user_id: userId, ...content }),
        JSON.stringify({ type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: String(content.summary ?? 'Standup update') }] }] }),
      ]
    )
  }

  // Get team member user IDs for assigning issues
  const aliceId = emailToUserId.get('alice.chen@ship.local')
  const bobId = emailToUserId.get('bob.martinez@ship.local')
  const davidId = emailToUserId.get('david.kim@ship.local')
  const emmaId = emailToUserId.get('emma.johnson@ship.local')
  const frankId = emailToUserId.get('frank.garcia@ship.local')

  // Get next available ticket number for demo issues
  const maxTicketResult = await pool.query(
    `SELECT COALESCE(MAX((properties->>'ticket_number')::int), 200) + 1 as next_num
     FROM documents d
     JOIN document_associations da ON da.document_id = d.id
  ...
  ) as { rows: Array<{ next_num: number }> }
  let ticketNum = maxTicketResult.rows[0]?.next_num ?? 201

  // ── Sprint 1: Active with Issues (rich analysis data) ─────────────────
  const activeSprint = await ensureDemoSprint(
    'FleetGraph Demo Week - Active with Issues',
    {
      ...
      success_criteria: 'All 7 analysis tools return grounded results. Action suggestions trigger dynamic dialogs.',
      confidence: 75,
      status: 'active',
    }
  )

  // Issues for active sprint — mix of states, priorities, assignees
  const activeIssues = [
    { title: 'Implement tool-calling analysis agent', state: 'done', priority: 'high', assignee: demoOwner.id, estimate: 8 },
    { title: 'Build 7 read-only analysis tools', state: 'done', priority: 'high', assignee: demoOwner.id, estimate: 6 },
    { title: 'Add HITL action suggestion flow', state: 'in_progress', priority: 'high', assignee: aliceId, estimate: 6 },
    { title: 'Create dynamic confirmation dialogs', state: 'in_progress', priority: 'medium', assignee: bobId, estimate: 4 },
    { title: 'Add proactive finding detection', state: 'in_progress', priority: 'high', assignee: davidId, estimate: 5 },
    { title: 'Write E2E tests for analysis flow', state: 'todo', priority: 'medium', assignee: emmaId, estimate: 4 },
    { title: 'Performance optimize tool execution', state: 'todo', priority: 'low', assignee: null, estimate: 3 },
    { title: 'Add streaming responses', state: 'backlog', priority: 'low', assignee: null, estimate: 6 },
  ]

  for (const issue of activeIssues) {
    const issueId = await ensureDemoIssue(issue.title, activeSprint, {
      state: issue.state,
      priority: issue.priority,
      source: 'internal',
      ...
      ticket_identifier: `SHIP-${ticketNum}`,
    }, ticketNum)
    ticketNum++

    // Add comments to the first 3 issues for evidence_lookup tool
    if (issue.state === 'done' || issue.state === 'in_progress') {
      await ensureDemoComment(issueId, demoOwner.id, `Started working on "${issue.title}". Architecture decision: using tool-calling pattern with schema validation.`)
      if (issue.assignee && issue.assignee !== demoOwner.id) {
        await ensureDemoComment(issueId, issue.assignee, `Making progress. Need to coordinate with the team on the API contract.`)
      }
    }
  }

  // Add standups for the active sprint
  if (aliceId) {
    await ensureDemoStandup(activeSprint, aliceId, {
      summary: 'Continued HITL action flow. Dialog rendering works for start_week and approve actions. Testing assign_owner next.',
      blockers: '',
      yesterday: 'Built dialog field definitions for 7 action types',
      today: 'Testing assign_owner with team member selection',
    })
  }
  if (bobId) {
    await ensureDemoStandup(activeSprint, bobId, {
      summary: 'Dynamic dialog generation is working well. Added form validation for text inputs.',
      blockers: 'Waiting on API endpoint for bulk issue assignment',
      yesterday: 'Implemented ConfirmDialog component integration',
      today: 'Adding error handling for failed action executions',
    })
  }

  // ── Sprint 2: Unassigned Issues (triggers unassigned_issues finding) ──
  const unassignedSprint = await ensureDemoSprint(
    'FleetGraph Demo Week - Unassigned Issues',
    {
      ...
      success_criteria: 'FleetGraph detects and surfaces the unassigned issue finding.',
      confidence: 60,
      status: 'active',
    }
  )
  ...
  const unassignedIssues = [
    { title: 'Set up monitoring dashboard', state: 'todo', priority: 'high', estimate: 6 },
    { title: 'Add alerting for API errors', state: 'todo', priority: 'high', estimate: 4 },
    { title: 'Create runbook for incident response', state: 'todo', priority: 'medium', estimate: 3 },
    { title: 'Implement health check endpoints', state: 'todo', priority: 'medium', estimate: 2 },
  ]

  for (const issue of unassignedIssues) {
    await ensureDemoIssue(issue.title, unassignedSprint, {
      state: issue.state,
      priority: issue.priority,
      source: 'internal',
      assignee_id: null,  // Intentionally unassigned
      estimate: issue.estimate,
      ticket_number: ticketNum,
      ticket_identifier: `SHIP-${ticketNum}`,
    }, ticketNum)
  ...
  // ── Sprint 3: No Owner (triggers sprint_no_owner finding) ────────────
  await ensureDemoSprint(
    'FleetGraph Demo Week - No Owner',
    {
      ...
      success_criteria: 'FleetGraph detects the missing owner and suggests the assign_owner action.',
      confidence: 40,
      status: 'planning',
    }
  )

  // ── Sprint 4: Plan Needs Approval (demonstrates approve_week_plan) ───
  await ensureDemoSprint(
    'FleetGraph Demo Week - Plan Needs Approval',
    {
      ...
      success_criteria: 'Export pipeline handles 10k+ rows. PDF generation under 5 seconds.',
      confidence: 80,
      status: 'planning',
      plan_approval: { status: 'submitted', submitted_at: new Date().toISOString() },
    }
  )

  console.log(`   Created 4 demo sprints with ${activeIssues.length + unassignedIssues.length} issues, comments, and standups`)
}
