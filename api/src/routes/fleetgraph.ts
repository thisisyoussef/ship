...
import { createToolCallingAdapter } from '../services/fleetgraph/llm/factory.js'
import { logFleetGraph } from '../services/fleetgraph/logging.js'
import { createChatOrchestrator } from '../services/fleetgraph/chat/orchestrator.js'
import { createChatSessionStore } from '../services/fleetgraph/chat/session.js'
import type { ChatSession, ChatTurnResult } from '../services/fleetgraph/chat/types.js'
...
interface FleetGraphRouterDeps {
  ...
  chatOrchestrator?: ReturnType<typeof createChatOrchestrator>
...
export function createFleetGraphRouter(
  ...
  // ── Chat Orchestrator (lazy singleton) ─────────────────────────────────
  let chatOrchestratorInstance: ReturnType<typeof createChatOrchestrator> | null = null

  function getChatOrchestrator(): ReturnType<typeof createChatOrchestrator> {
    if (deps.chatOrchestrator) return deps.chatOrchestrator
    if (chatOrchestratorInstance) return chatOrchestratorInstance

    const llmConfig = resolveLLMConfig(process.env)
    if (!llmConfig) {
      throw new Error('LLM configuration not available for chat orchestrator')
    }
    const toolCallingAdapter = createToolCallingAdapter(llmConfig)
    const sessionStore = createChatSessionStore()
    chatOrchestratorInstance = createChatOrchestrator({ llm: toolCallingAdapter, sessionStore })
    return chatOrchestratorInstance
  }

  function mapChatTurnToThreadResponse(
    result: ChatTurnResult,
    chatThreadId: string,
    session: ChatSession
  ) {
    const hasPending = result.pendingApproval !== null
    const hasActions = result.actionDrafts.length > 0
    ...
    return {
      ...
      responsePayload: {
        ...
        answer: {
          text: result.analysisNarrative,
          entityLinks: [] as unknown[],
          suggestedNextSteps: [] as string[],
        },
      },
      actionDrafts: result.actionDrafts,
      pendingApproval: result.pendingApproval ? {
        id: result.pendingApproval.toolCallId,
        title: `${result.pendingApproval.actionType}`,
        summary: result.pendingApproval.rationale,
        actionDraft: result.pendingApproval.actionDraft,
        dialogSpec: result.pendingApproval.dialogSpec,
      } : null,
      reasonedFindings: result.reasonedFindings.map(f => ({
        ...
        affectedPerson: f.affectedPerson,
      })),
  ...
  router.post('/chat/start', authMiddleware, async (req: Request, res: Response) => {
    const auth = getAuthContext(req, res)
    if (!auth) {
      return
    }
    ...
    try {
      ...
      const requestContext = buildShipRestRequestContext(req)
      const result = await orchestrator.startChat({
        workspaceId: auth.workspaceId,
        actorId: auth.userId,
        documentId,
        documentType,
      ...
      const chatThreadId = `fleetgraph:chat:${auth.workspaceId}:${documentId}`
      const session = orchestrator.getSession(chatThreadId)
      if (!session) {
        res.status(500).json({ error: 'Chat session could not be created' })
        return
      }

      res.json(mapChatTurnToThreadResponse(result, chatThreadId, session))
    } catch (error) {
      console.error('FleetGraph chat start error:', error)
      res.status(500).json({ error: 'Failed to start FleetGraph chat' })
    }
  })

  router.post('/chat/:threadId/message', authMiddleware, async (req: Request, res: Response) => {
    const auth = getAuthContext(req, res)
    if (!auth) {
      return
    }
    ...
    try {
      ...
      console.error('FleetGraph chat message error:', error)
      res.status(500).json({ error: 'Failed to process chat message' })
    }
  })

  router.post('/chat/:threadId/approve', authMiddleware, async (req: Request, res: Response) => {
    const auth = getAuthContext(req, res)
    if (!auth) {
      return
    }
    ...
    try {
      ...
      if (!session.pendingApproval) {
        res.status(400).json({ error: 'No action is pending approval' })
        return
      }

      const submission: FleetGraphDialogSubmission = {
        actionId: session.pendingApproval.actionDraft.actionId,
        values: values ?? {},
      }
      ...
      const updatedSession = orchestrator.getSession(chatThreadId)
      if (!updatedSession) {
        res.status(500).json({ error: 'Session lost after approval' })
        return
      }

      res.json(mapChatTurnToThreadResponse(result, chatThreadId, updatedSession))
    } catch (error) {
      console.error('FleetGraph chat approve error:', error)
      res.status(500).json({ error: 'Failed to approve chat action' })
    }
  })

  router.post('/chat/:threadId/dismiss', authMiddleware, async (req: Request, res: Response) => {
    const auth = getAuthContext(req, res)
    if (!auth) {
      return
    }
    ...
    try {
      ...
      console.error('FleetGraph chat dismiss error:', error)
      res.status(500).json({ error: 'Failed to dismiss chat action' })
    }
  })

  router.get('/chat/:threadId', authMiddleware, async (req: Request, res: Response) => {
    const auth = getAuthContext(req, res)
    if (!auth) {
      return
    }
    ...
    try {
      ...
      res.json({
        ...
        completedActions: session.completedActions.length,
        createdAt: session.createdAt,
        lastActiveAt: session.lastActiveAt,
      })
    } catch (error) {
      console.error('FleetGraph chat session error:', error)
