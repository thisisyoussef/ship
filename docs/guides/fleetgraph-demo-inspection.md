...
15. Open `FleetGraph Demo Week - Validation Ready`.
16. Open the `Review` tab for `FleetGraph Demo Week - Validation Ready`.
17. Click `Check this page` in the entry card and inspect the FAB handoff.
18. Ask a follow-up such as `What else should I look at?` in the FAB.
19. Click `Preview next step` in the entry card and inspect the guided-step preview state.
20. Open `FleetGraph debug`.
21. Inspect the secondary debug dock for thread history and pending interrupts.
22. Return to `Documents`.
23. Open `FleetGraph Demo Week - Worker Generated`.
...
- Re-running the public demo bootstrap resets `FleetGraph Demo Week - Owner Gap` back to an `active` sprint with no owner assigned.
