...
15. Open `FleetGraph Demo Week - Unassigned Issues`.
16. Inspect the proactive FleetGraph card and confirm the unassigned-issues finding is visible.
17. Confirm the suggested next step is advisory-only and does not expose `Review and apply`.
...
## Expected Unassigned-Issues Flow
...
- Re-running the public demo bootstrap resets `FleetGraph Demo Week - Unassigned Issues` back to an `active` sprint with three unassigned issues and two assigned issues.
