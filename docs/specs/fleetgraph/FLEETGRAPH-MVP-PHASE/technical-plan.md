...
- Story ordering rationale:
  ...
  - `T104A` repairs the public demo proof lane by ensuring FleetGraph schema/bootstrap and repeatable seeded inspection targets on Railway, so future stories can be monitored from prod instead of relying on lucky data or skipped predeploy hooks.
  - `T104B` deploys the real FleetGraph worker lane on Railway, preserves the stable seeded HITL proof lane, and adds a second named inspection target whose proactive finding is generated by the live worker path.
  - `T105` captures the shared traces, public deploy proof, and final `FLEETGRAPH.md` evidence sections once the seeded and worker-generated deployed paths both exist.
- Gaps/overlap check:
  ...
  - The pack now reserves one explicit story for seeded demo inspectability and one explicit story for deployed worker execution, because the public demo cannot be a trustworthy MVP audit surface if visible proof is only seeded and never produced by the live worker path.
...
- Risk: the public demo can report healthy while FleetGraph tables or visible proof data are missing, which makes UI inspection misleading.
  ...
  - Risk: the public demo may still show FleetGraph UI proof without proving autonomous proactive execution, because the visible finding could come only from seed/bootstrap state.
  - Mitigation: add `T104B` to deploy a dedicated Railway worker lane and verify a second named finding created by the live worker path.
