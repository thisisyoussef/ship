...
```json
{
  ...
  "draft": {
    "requestedAction": {
      "type": "approve_project_plan|approve_week_plan|assign_issues|assign_owner|post_comment|start_week|validate_week_plan",
      "endpoint": { "method": "POST|PATCH|DELETE", "path": "/api/..." },
      "targetId": "string",
...
Current service-acceptance nuance:

1. The shared action schema includes `post_comment`.
2. The current entry service only accepts action types wired in `ACTION_SHAPE_BY_TYPE`:
   ...
   - `start_week`
   - `validate_week_plan`
3. Unsupported draft actions are rejected with `400`.
...
```json
{
  ...
  "approval": {
    ...
    "evidence": ["string"],
    "options": [
      { "id": "apply", "label": "string" },
      { "id": "dismiss", "label": "string" },
      { "id": "snooze", "label": "string" }
    ],
    "rationale": "string",
  ...
  "entry": {
    "current": { "documentType": "string", "id": "string", "title": "string" },
    "route": { "surface": "string", "activeTab": "optional string", "nestedPath": ["string"] },
    "threadId": "string"
  },
  "run": {
    "branch": "string",
...
### Proactive findings routes

| Endpoint | Request body | Success response |
| --- | --- | --- |
| `GET /api/fleetgraph/findings` | optional `documentIds` query param | `{ findings: [...] }` |
| `POST /api/fleetgraph/findings/:id/dismiss` | none | `{ finding }` |
| `POST /api/fleetgraph/findings/:id/review` | optional `{ assigneeId?, ownerId? }` | `{ finding, review: { title, summary, evidence, confirmLabel, cancelLabel, threadId } }` |
| `POST /api/fleetgraph/findings/:id/snooze` | `{ minutes?, seconds? }` | `{ finding }` |
| `POST /api/fleetgraph/findings/:id/apply` | optional `{ assigneeId?, ownerId? }` | `{ finding }` |

`finding` payloads use:
