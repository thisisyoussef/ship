...
await retry(async () => {
  const response = await fetch(`${trimTrailingSlash(callbackBaseUrl)}/api/internal/runs/${runId}/events`, {
    method: 'POST',
    headers: {
      ...
      events: [
        {
          ...
          type: process.env.AUDIT_EVENT_TYPE || 'workflow-note',
          level: process.env.AUDIT_EVENT_LEVEL || 'info',
          phase: process.env.AUDIT_EVENT_PHASE || 'setup',
          message: readRequiredEnv('AUDIT_EVENT_MESSAGE'),
          payload: {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Event callback failed (${response.status}): ${errorText || response.statusText}`);
  }
});
